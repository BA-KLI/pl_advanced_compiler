    ...
    void insertBeforeStmt(  PLpgSQL_stmt  *oldStmt,
                            PLpgSQL_stmt  *newStmt,
                            List          *parents);
    ...
    PLpgSQL_stmt_execsql *createExecsql(  char *query,
                                          bool into,
                                          PLpgSQL_rec* rec,
                                          PLpgSQL_row* row);

    List* handleFors(PLpgSQL_stmt_fors *fors ,
                    List               *parents,
                    PLpgSQL_execstate  *estate,
                    PLpgSQL_function   *func,
                    igraph_t           *igraph
    ...
    List *resolveQueryRefsSkipNFull(char  *query,
        ...
        Node* node = lfirst(raw_parser(query)->head);
        SelectStmt *select;
    ...
    /**
     * Creates a execsql statement
     * @param  query the query of the exec stmt
     * @param  into  into statement?
     * @param  rec   record
     * @param  row   row
     * @return       the execsql statement
     */
    PLpgSQL_stmt_execsql *createExecsql(  char *query,
                                          bool into,
                                          PLpgSQL_rec* rec,
                                          PLpgSQL_row* row) {
        PLpgSQL_stmt_execsql *newExecSql = palloc0(sizeof(PLpgSQL_stmt_execsql));
        newExecSql->cmd_type          = PLPGSQL_STMT_EXECSQL;
        newExecSql->sqlstmt           = palloc0(sizeof(PLpgSQL_expr));
        newExecSql->sqlstmt->dtype      = PLPGSQL_DTYPE_EXPR;
        newExecSql->sqlstmt->query      = query;
        newExecSql->mod_stmt            = true;
        newExecSql->into                = into;
        newExecSql->rec                 = rec;
        newExecSql->row                 = row;
        newExecSql->strict              = false;
        return newExecSql;
    }
    ...
    /**
     * inserts newStmt before oldStmt
    ...
    void insertBeforeStmt(  PLpgSQL_stmt  *oldStmt,
        ...
        if(linitial(parents) == oldStmt){
            printf("here\n");
            lcons(newStmt,parents);
            return;
        }
        ...
        foreach(l, parents) {
            if (l->next != NIL && oldStmt == lfirst(l->next)) {
                lappend_cell(parents,l,newStmt);
                break;
            }
        }
    ...
    List* handleFors(PLpgSQL_stmt_fors *fors ,
        ...
        /* Find the first statement that is a PLPGSQL_STMT_EXECSQL statement */

        ListCell* cell = fors->body->head;

        PLpgSQL_stmt *firstStmt = NULL;

        while(cell != NULL && lfirst(cell) != NULL){
            PLpgSQL_stmt *curStmt = lfirst(cell);

            if(curStmt->cmd_type == PLPGSQL_STMT_EXECSQL){
                firstStmt = curStmt;
                break;
            }
        ...
        while(cell != NULL && lfirst(cell) != firstStmt){
            if(conflict(lfirst(cell),firstStmt,igraph)){
                return parents;
            }
            cell = cell->next;
        }

        lfirst(cell) = lfirst(fors->body->head);
        lfirst(fors->body->head) = firstStmt;
            ...
            */
            ...
            if(execsql->into){
                ...
                */
                ...
                Node* node = lfirst(raw_parser(execsql->sqlstmt->query)->head);
                ...
                if(IsA(node,InsertStmt)){
                    ...
                    //TODO Schema
                    char* insertIntoName = insert->relation->relname;

                    /**
                     * Project to the columns c1,...,cn,c′′1,...,c′′k
                     */
                    sprintf(qbquery, "INSERT INTO %s SELECT %s FROM %s",
                            insertIntoName,
                            concatValuesWithPrefix(c__, ",", "__c__"),
                            leftOuterJoin(
                                qO->query,
                    ...
                    PLpgSQL_stmt_execsql* newInto = createExecsql(qbquery,0,NIL,NIL);
                    //execsql->sqlstmt->query = qbquery;


                    insertBeforeStmt((PLpgSQL_stmt *)fors,(PLpgSQL_stmt *)newInto, parents);
                    fors->body = list_delete(fors->body,execsql);


                    if(list_length(fors->body) == 0){
                        parents = list_delete(parents,fors);
                    }
                }
        ...
        foreach(l, list_copy(statements)) {
            ...
            switch (stmt->cmd_type) {
                case PLPGSQL_STMT_FORS: {
                    func->action->body = handleFors((PLpgSQL_stmt_fors  *)  stmt,
                               statements,
                               estate,
                               func,
                               igraph);
